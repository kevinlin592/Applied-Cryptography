...
/// process 64 bytes
void Hashing::SHA256::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void Hashing::SHA256::add(const void* data, size_t numBytes)
{
...
void Hashing::SHA256::processBuffer()
{
...
std::string Hashing::SHA256::getHash()
{
...
void Hashing::SHA256::getHash(unsigned char buffer[SHA256::HashBytes])
{
...
std::string Hashing::SHA256::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string Hashing::SHA256::operator()(const std::string& text)
{
  reset();
